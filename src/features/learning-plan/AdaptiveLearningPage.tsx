...
const goalTitle = (goal: AdaptiveLearningGoal) =>
  GOALS.find((item) => item.value === goal)?.title ?? 'Adaptive goal';
...
function SetupView({
  ...
  diagnosticSource?: AdaptiveDiagnostic['source'];
  ...
  return (
    <DemoWhitePanel className="grid lg:grid-cols-[0.8fr_1.2fr]">
      <section className="border-b border-black/10 bg-[#102b26] p-6 text-white lg:border-b-0 lg:border-r lg:p-8">
        ...
        {diagnosticSource ? (
          <p className="mt-5 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs text-white/80">
            <Sparkles size={13} className="text-[#d9f99d]" aria-hidden="true" />
            {diagnosticSource === 'GEMINI'
              ? 'Fresh 10-question set generated by Gemini'
              : 'Fresh 10-question set from the verified question bank'}
          </p>
        ) : null}
      </section>
      ...
      <aside className="space-y-4">
        <div className="rounded-[1.25rem] bg-[#102b26] p-5 text-white">
          ...
          <Button
            onClick={onGenerate}
            loading={generating}
            className="mt-5 w-full border-black bg-black text-white hover:border-zinc-800 hover:bg-zinc-800"
          >
            Generate roadmap
          </Button>
          {generateError ? (
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_20rem]">
      <div className="space-y-5">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full bg-black/[0.045] px-3 py-1 text-xs font-medium text-black/65">
                  {goalTitle(plan.goal)}
                </span>
        ...
        <DemoWhitePanel className="p-5 sm:p-7">
          ...
          <section className="mb-7 grid gap-3 rounded-2xl border border-black/10 bg-black/[0.025] p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#0b7668]">
                {plan.scope === 'FULL_COURSE' ? 'Full course roadmap' : 'Focused roadmap'}
              </p>
              <p className="mt-1 text-sm text-black/60">
                {plan.scope === 'FULL_COURSE'
                  ? 'Includes every remaining course lesson in curriculum order.'
                  : 'Selects lessons for this goal and your current mastery.'}
              </p>
            </div>
            <div className="rounded-xl bg-white px-4 py-3 text-left shadow-sm sm:text-right">
              <p className="text-xl font-semibold tracking-[-0.03em] text-[#102b26]">
                {plan.coverage.selectedLessons}/{plan.coverage.totalRemainingLessons}
              </p>
              <p className="text-[11px] text-black/50">remaining lessons covered</p>
            </div>
          </section>
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-black/55">
                Selected lessons keep the instructor-authored curriculum order.
              </p>
            </div>
                ...
                <section className="rounded-2xl border border-black/10 p-4 sm:p-5">
                  ...
                  <div className="mt-4 space-y-2">
                    {week.lessons.map((lesson) => {
                      const isCompleted = lesson.isCompleted ?? lesson.isReview;
                      const content = (
                        <span className="flex min-w-0 items-center gap-3">
                          ...
                          {isCompleted ? (
                            ...
                            <span className="rounded-full bg-black/5 px-2 py-0.5 text-[10px] text-black/55">Review</span>
                          ) : null}
                        </span>
                      );
                      ...
                      return (
                        <Link
                          ...
                          className={`flex min-h-12 items-center justify-between gap-3 rounded-xl px-3 py-2 text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0b7668]/30 ${
                            isCompleted
                              ? 'bg-emerald-500/[0.08] hover:bg-emerald-500/[0.14]'
                              : 'bg-black/[0.035] hover:bg-[#d9f99d]/35'
                          }`}
                        >
                          {content}
                          <ChevronRight
...
export const AdaptiveLearningPage: React.FC = () => {
  ...
  const [assessmentSequence, setAssessmentSequence] = useState(0);

  const diagnosticQuery = useQuery({
    queryKey: ['adaptive-diagnostic', COURSE_SLUG, assessmentSequence],
  ...
  const openAssessmentSetup = () => {
    ...
    setQuestionIndex(0);
    setAnswers({});
    setAssessmentSequence((sequence) => sequence + 1);
    setView('setup');
  };
